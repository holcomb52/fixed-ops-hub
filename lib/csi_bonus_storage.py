...
from lib.csi_bonus_calc import calculate_csi_bonus, inputs_from_snapshot, result_to_dict
...
ARCHIVE_DIR = Path(__file__).resolve().parent.parent / "data" / "csi_bonus_archive"
TABLE = "csi_bonus_runs"
...
def serialize_csi_bonus_session(
    *,
    employee_name: str,
    bonus_month: str,
    store_nps: float,
    ...
    result = calculate_csi_bonus(
        ...
        business_center_average=business_center_average,
        notes=notes,
...
def apply_csi_bonus_snapshot_to_session(snapshot: dict, run_id: str, status: str = "completed"):
    inputs = inputs_from_snapshot(snapshot)
    st.session_state.active_csi_bonus_run_id = run_id
    st.session_state.csi_bonus_completed = status == "completed"
    st.session_state.csi_employee_name = inputs["employee_name"]
    st.session_state.csi_bonus_month_label = inputs["bonus_month"]
    st.session_state.csi_store_nps = inputs["store_nps"]
    st.session_state.csi_national_average = inputs["national_average"]
    st.session_state.csi_business_center_average = inputs["business_center_average"]
    st.session_state.csi_notes = inputs["notes"]
    st.session_state.csi_paid_on = inputs["paid_on_label"]
    month_label = (inputs.get("bonus_month") or "").strip()
    if month_label:
        try:
            st.session_state.csi_month = datetime.strptime(month_label, "%B %Y").date().replace(day=1)
        except ValueError:
            pass
...
def save_csi_bonus_run(
    ...
    *,
    ...
    record = {
        ...
        "status": status,
        "snapshot": snapshot,
        "grand_total": snapshot.get("bonus_amount", 0),
...
def list_csi_bonus_runs() -> List[dict]:
    runs: dict = {}
    ...
    if client:
        try:
            result = (
                client.table(TABLE)
                .select(
                    "id,pay_period,status,grand_total,employee_name,completed_at,updated_at"
                )
...
def load_csi_bonus_run(run_id: str) -> Optional[dict]:
    client = get_supabase()
    if client:
...
def delete_csi_bonus_run(run_id: str) -> Tuple[bool, str]:
    if not run_id:
