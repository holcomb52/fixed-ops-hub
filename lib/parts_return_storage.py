"""Persist parts return allowance plans for Reports."""
...
ARCHIVE_DIR = Path(__file__).resolve().parent.parent / "data" / "parts_return_archive"
TABLE = "parts_return_runs"
...
def save_parts_return_run(
    ...
    *,
    ...
    record = {
        ...
        "status": status,
        "snapshot": snapshot,
        "grand_total": float(snapshot.get("selected_value", 0) or 0),
        "tech_count": float(snapshot.get("selected_count", 0) or 0),
        "completed_at": now,
        "updated_at": now,
    }
...
def list_parts_return_runs() -> List[dict]:
    runs: dict = {}
...
def load_parts_return_run(run_id: str) -> Optional[dict]:
    client = get_supabase()
    if client:
...
def delete_parts_return_run(run_id: str) -> Tuple[bool, str]:
    if not run_id:
